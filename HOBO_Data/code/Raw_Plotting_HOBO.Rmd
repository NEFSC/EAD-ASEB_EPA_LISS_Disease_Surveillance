...
df2<-read_csv("HOBO_Data\\raw_data\\112024_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
  dplyr::rename(Temperature = `Temperature`) %>%
  dplyr::rename(Date_Time = `Date-Time (EST/EDT)`)# %>%
  dplyr::select( c( "Temperature", "Date_Time")) 
...
FENC_HOBO <- rbind(df1,df2,df3)   #Combine the two data sheets
...
df9<-read_csv("HOBO_Data\\raw_data\\1225_ASHC_HOBO.csv", col_names = TRUE)%>%  
  dplyr::rename(Temperature = `Temperature`) %>%
