...
Last updated: 10/16/25 M.Kachmar
...
df2<-read_csv("HOBO_Data\\raw_data\\102025_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
...
df9<-read_csv("HOBO_Data\\raw_data\\082025_ASHC_HOBO.csv", col_names = TRUE)%>%  
  dplyr::rename(Temperature = `Temperature`) %>%
  dplyr::rename(Date_Time = `Date-Time (EDT)`) #%>% 
  #dplyr::select( -c( "Button Down", "Button Up", "End of File", "Started Offset","Host Connected"))
...
GOLD_df2 <- read_csv("HOBO_Data\\raw_data\\092025_GOLD_HOBO.csv", col_names = TRUE) 
GOLD_df2 <- GOLD_df2 %>%
  dplyr::rename(Temperature = `Temperature`) %>%
  dplyr:: rename(Date_Time = `Date-Time (EDT)`)%>%
