...
Last updated: 11/14/25 M.Kachmar
...
df2<-read_csv("HOBO_Data\\raw_data\\112025_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
...
df9<-read_csv("HOBO_Data\\raw_data\\112025_ASHC_HOBO.csv", col_names = TRUE)%>%  
  dplyr::rename(Temperature = `Temperature`) %>%
...
GOLD_df2 <- read_csv("HOBO_Data\\raw_data\\102025_GOLD_HOBO.csv", col_names = TRUE) 
GOLD_df2 <- GOLD_df2 %>%
  dplyr::rename(Temperature = `Temperature`) %>%
  dplyr:: rename(Date_Time = `Date-Time (EDT)`)%>%
