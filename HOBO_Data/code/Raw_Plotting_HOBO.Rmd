...
df2<-read_csv("HOBO_Data\\raw_data\\052025_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
...
df8<-read_csv("HOBO_Data\\raw_data\\052025_ASHC_HOBO.csv", col_names = TRUE)%>%  
  dplyr::rename(Temperature = `Temperature`) %>%
  dplyr::rename(Date_Time = `Date-Time (EST/EDT)`) #%>% 
  #dplyr::select( -c( "Button Down", "Button Up", "End of File", "Started Offset","Host Connected"))
