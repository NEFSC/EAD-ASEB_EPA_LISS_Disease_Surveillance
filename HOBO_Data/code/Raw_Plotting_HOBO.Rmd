...
Last updated: 6/18/25 M.Kachmar
...
df2<-read_csv("HOBO_Data\\raw_data\\062025_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
...
ASHC_HOBO <- rbind(df3,df4,df5,df6,df7, df8)
...
GOLD_df2 <- read_csv("HOBO_Data\\raw_data\\062025_GOLD_HOBO.csv", locale = locale(encoding = "Latin1")) 
GOLD_df2 <- GOLD_df2 %>%
  dplyr::rename(Temperature = `Temperature , °C`) %>%
  dplyr:: rename(Date_Time = `Date-Time (EDT)`)
GOLD_df2
...
GOLD_HOBO <- dplyr::bind_rows(GOLD_df1,GOLD_df2)
