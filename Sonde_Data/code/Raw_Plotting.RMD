...
Last updated 05/04/2026 by M.Kachmar
...
dev.off()              theme(legend.position = "none", axis.text= element_text(size= 15))+scale_x_datetime(breaks="1 month", date_labels = "%B")
...
FENC_CHLA_plot <- FENC_CHLA %>% na.omit() %>%
             dplyr::filter(Date.Time > '2026-01-01 00:01:00' & Date.Time < '2026-01-31 23:59:00' ) %>% # change date range needed here
             dplyr::mutate(Site = case_when(Site == 'FENC' ~ 'Fence Creek', .default = 'Ash Creek')) %>% 
             ggplot(aes(x=Date.Time, 
                       y=mean, # the mean values
                       group=Site)) + # call the new mean
...
GOLD_pH_ALL <- GOLD_pH %>% na.omit() %>%
             dplyr::filter(Date.Time > '2023-04-01 00:01:00' & Date.Time < '2026-12-31 23:59:00' ) %>% # change date range needed here
             dplyr::mutate(Site = case_when(Site == 'GOLD' ~ 'Gold Star', .default = 'Fence Creek')) %>% 
             ggplot(aes(x=Date.Time, 
                       y=mean, # the mean values
                       group=Site)) + # call the new mean
