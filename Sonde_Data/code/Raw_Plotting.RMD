...
Master_table_cleaned <- Master_table %>%
  ...
  mutate(
    ...
    temp_clean = ifelse(Temperature < -2 | Temperature > 40, NA, Temperature),
    temp_delta = abs(temp_clean - lag(temp_clean, default = first(temp_clean))),
    temp_clean = ifelse(temp_delta > 2.0, NA, temp_clean),
    ...
    sal_clean = ifelse(sal_delta > 5.0, NA, sal_clean),
    ...
    ph_clean = ifelse(ph_delta > 0.5, NA, ph_clean),
    ...
    do_roll_sd = rollapply(do_clean, width = 8, FUN = sd, fill = NA, align = "right"),
...
Master_table_cleaned <- Master_table_cleaned %>%
  ...
  mutate(
    ...
    # Use a relaxed 3 * IQR multiplier for CHLA to prevent erasing real biological blooms
...
QAQC_plot<- df_clean_plot %>%
  dplyr::filter(parameter %in% c("RDO Concentration")) %>%
 ...
 # Facet by Site only (since parameter is now filtered to one value)
  ...
  labs(
    ...
    x = "Date Time",
    color = "Data Status",
    title = "Intertidal RDO Concentration"
  ) +
  theme_minimal(base_size = 11) + 
  theme(
    legend.position = "top", 
...
pdf(paste0(path = "output/raw_plots","/DO_QAQC.pdf"), height = 6, width = 11) 
print(QAQC_plot)
dev.off()
