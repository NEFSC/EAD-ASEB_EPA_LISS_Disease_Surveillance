...
Last updated 4/4/25 by M.Kachmar
...
CT_means_filterCT_means_filterd %>% dplyr::filter(!mean =="231.92899")
...
pdf(paste0(path = "C:/Users/mariah.kachmar/documents/Github/EAD-ASEB_EPA_LISS_Disease_Surveillance/Side_projects/Juvenile_growth/outputs/" ,"CT.data.plot.pdf"), height = 7, width = 11) #printing a PDF within the path of the code, with specific dimensions of PDF
...
ASHC_pH_2024 <- ASHC_pH %>% na.omit() %>%
             dplyr::filter(`Date.Time` > '2024-03-01 00:00:00' & `Date.Time` < '2025-11-30 00:00:00' ) %>%  # change date range needed here
             dplyr::mutate(Site = case_when(Site == 'ASHC' ~ 'Ash Creek', .default = 'Fence Creek')) %>% 
             ggplot(aes(x=`Date.Time`, 
                       y=mean, # the mean values
                       group=Site)) + # call the new mean
...
FENC_pH_2024 <- FENC_pH %>% na.omit() %>%
             dplyr::filter(`Date.Time` > '2024-03-01 00:00:00' & `Date.Time` < '2025-11-30 00:00:00' ) %>% 
             dplyr::mutate(Site = case_when(Site == 'FENC' ~ 'Fence Creek', .default = 'Ash Creek')) %>% 
             ggplot(aes(x=`Date.Time`, 
                       y=mean, # the mean values
                       group=Site)) + # call the new mean
...
Temp.rest <- TempvspH %>% filter(Year!="2023") %>% filter(variable=="Temperature")
pH.2023 <- TempvspH %>% filter(Year=="2023") %>% filter(variable=="pH")
pH.rest <- TempvspH %>% filter(Year!="2023") %>% filter(variable=="pH")
...
All_temp.graph <-ggplot()+
  ...
  geom_line(data=Temp.2023%>% filter(Site == "GOLD"), aes(x=`Date.Time`, #GOLD on top
                       y=mean, color = Site))+
  geom_line(data=Temp.rest%>% filter(Site == "FENC"), aes(x=`Date.Time`, #second half of the graph (2024-2025)
                       y=mean, color = Site))+
  geom_line(data=Temp.rest%>% filter(Site == "ASHC"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
  geom_line(data=Temp.rest%>% filter(Site == "GOLD"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
...
All_pH.graph <-ggplot()+
  ...
  geom_line(data=pH.2023%>% filter(Site == "GOLD"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
  geom_line(data=pH.rest%>% filter(Site == "FENC"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
  geom_line(data=pH.rest%>% filter(Site == "ASHC"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
  geom_line(data=pH.rest%>% filter(Site == "GOLD"), aes(x=`Date.Time`, 
                       y=mean, color = Site))+
scale_color_manual(values=c("FENC" = "darksalmon",ASHC = "cornflowerblue", "GOLD" = "darkseagreen")) +
