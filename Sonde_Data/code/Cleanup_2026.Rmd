...
file.names.2026         <- data.frame(txt.files = 
...
for (m in 1:nrow(file.names.2026)) { # this is your loop, for each row in th filenames tbale, do the followin!
  ...
  raw_Sonde_rootdir        <- paste0(getwd(),"/",path.raw,file.names.2026[m,1]) #reads in the data files
  ...
  filename      <- file.names.2026[m,1]
  raw_df        <- raw_df %>% 
                    dplyr::mutate(Site = sub(".*_(.*)_.*","\\1",filename),
                    Date = gsub("_.*","",filename)) 
  
  Master_2026 <- dplyr::bind_rows(raw_df, Master_2026) #bind to a cumulative list dataframe , changed from rbind() to bind_rows due to the new sonde 1335676 recording depth data the older models do not record. 
}
...
Master_2026_output <- Master_2026 %>%  
                      dplyr::select(-c("External Voltage", "Battery Capacity", "Barometric Pressure", "Date")) 
Master_2026_output <- Master_2026_output %>% filter(!is.na(Site))


write.csv(Master_2026_output, "output/Sonde Master Files/Sonde_master_2026.csv")
```