...
Last updated 09/9/2026 by M.Kachmar
...
Master_table_LONG_means <- Master_table_cleaned %>%  
                      ...
                      reshape2::melt(id.vars = c("Date.Time", "Site"),
                                    measure.vars = c("Actual.Conductivity", "sal_clean", "Resistivity", 
                                                     "Total.Dissolved.Solids" ,"do_clean", "RDO.Saturation","Oxygen.Partial.Pressure", "chla_clean", "ph_clean", "pH.mV", 
                                                     "temp_clean", "Depth", "Pressure")) %>% 
    ...
    .groups = "drop")%>%
                      ...
                      dplyr::mutate(Date.Time = lubridate::ymd_h(Date.Time)) # reformat back to date 
...
# Match the names in vars$dat to the actual names in Master_table_LONG_means
variable_map <- c(
  "Actual Conductivity"       = "Actual.Conductivity",
  "Salinity"                  = "sal_clean",
  "Resistivity"               = "Resistivity",
  "Total Dissolved Solids"    = "Total.Dissolved.Solids",
  "RDO Concentration"         = "do_clean",
  "RDO Saturation"            = "RDO.Saturation",
...
for(i in 1:nrow(vars)) {
  ...
  # Display name from vars
  display_var <- as.character(vars$dat[i])
  
  # Actual variable name in Master_table_LONG_means
  current_var <- variable_map[display_var]
  ...
  if(is.na(current_var)) {
    message("No variable mapping found for: ", display_var)
    next
  }
  ...
  Table_by_var <- Master_table_LONG_means %>% 
    dplyr::filter(
      as.character(variable) == current_var
    )
  
  print(nrow(Table_by_var))
  
  # Plot only if data exist
  if(nrow(Table_by_var) == 0) {
    message("No data found for: ", current_var)
    next
  }
...
```
#Master files for temperature validation with HOBO
```{r}

ASHC_Temperature_Sonde <- Master_table_cleaned %>% dplyr::filter(Site %in% c('ASHC')) %>% select(Date.Time, temp_clean, Site)
FENC_Temperature_Sonde <- Master_table_cleaned %>% dplyr::filter(Site %in% c('FENC')) %>% select(Date.Time, temp_clean, Site)
GOLD_Temperature_Sonde <- Master_table_cleaned %>% dplyr::filter(Site %in% c('GOLD')) %>% select(Date.Time, temp_clean, Site)

write.csv(ASHC_Temperature_Sonde, "output/Sonde Master Files/ASHC_Temperature_Sonde.csv")
write.csv(FENC_Temperature_Sonde, "output/Sonde Master Files/FENC_Temperature_Sonde.csv")
write.csv(GOLD_Temperature_Sonde, "output/Sonde Master Files/GOLD_Temperature_Sonde.csv")
...
CT_pH          <- Master_table_LONG_means %>% dplyr::filter(variable == 'ph_clean' & Site %in% c('FENC', 'ASHC'))
CT_Salinity    <- Master_table_LONG_means %>% dplyr::filter(variable == 'sal_clean' & Site %in% c('FENC', 'ASHC'))
CT_Temperature <- Master_table_LONG_means %>% dplyr::filter(variable == 'temp_clean' & Site %in% c('FENC','ASHC'))
CT_DO         <- Master_table_LONG_means %>% dplyr::filter(variable == 'do_clean' & Site %in%c('FENC','ASHC'))
...
ASHC_pH          <- Master_table_LONG_means %>% dplyr::filter(variable == 'ph_clean' & Site %in% c('ASHC'))
ASHC_Salinity    <- Master_table_LONG_means %>% dplyr::filter(variable == 'sal_clean' & Site %in% c('ASHC'))
ASHC_Temperature <- Master_table_LONG_means %>% dplyr::filter(variable == 'temp_clean' & Site %in% c('ASHC'))
ASHC_DO          <- Master_table_LONG_means %>% dplyr::filter(variable == 'do_clean' & Site %in% c('ASHC'))
ASHC_CHLA        <- Master_table_LONG_means %>% dplyr::filter(variable == 'chla_clean' & Site %in% c('ASHC'))

### Update each graph for the time frame desired as well as the name of the file to coincide with the appropriate year displayed

#write.csv(ASHC_Temperature, "output/Sonde Master Files/ASHC_Temperature_Sonde.csv")
...
ASHC_Monthly_Temperature <- ASHC_Temperature %>% na.omit() %>%
  ...
  dplyr::summarize(Monthly_mean = mean(mean), sd=sd(sd))
ASHC_Monthly_Temperature

```
```{r}
#Overlay of temp by year - ASHC
ashc_temp<- ggplot(ASHC_Monthly_Temperature, aes(x=Month, y=Monthly_mean, group = Year, fill = factor(Year)))+
  geom_line(linewidth=0.75) +
  geom_ribbon(aes(ymax = Monthly_mean+sd, ymin = Monthly_mean-sd), alpha=0.15)+
...
#write.csv(FENC_Temperature, "output/Sonde Master Files/FENC_Temperature_Sonde.csv")
...
```
```{r}
#Overlay of temp and salinity by year - FENC

fenc_temp<- ggplot(FENC_Monthly_Temperature, aes(x=Month, y=Monthly_mean, group = Year, fill = factor(Year)))+
  geom_line(linewidth=0.75) +
  geom_ribbon(aes(ymax = Monthly_mean+sd, ymin = Monthly_mean-sd), alpha=0.15)+
  labs(x="Month", y="Temperature", colour="Year") +
  theme_classic()
fenc_temp

fenc_sal<- ggplot(FENC_Monthly_Salinity, aes(x=Month, y=Monthly_mean, group = Year, fill = factor(Year)))+
  geom_line(linewidth=0.75) +
  geom_ribbon(aes(ymax = Monthly_mean+sd, ymin = Monthly_mean-sd), alpha=0.15)+
  labs(x="Month", y="Salinity", colour="Year") +
  theme_classic()
fenc_sal
...
GOLD_pH          <- Master_table_LONG_means %>% dplyr::filter(variable == 'ph_clean' & Site %in% c('GOLD')) 
GOLD_Salinity    <- Master_table_LONG_means %>% dplyr::filter(variable == 'sal_clean' & Site %in% c('GOLD'))
GOLD_Temperature <- Master_table_LONG_means %>% dplyr::filter(variable == 'temp_clean' & Site %in% c('GOLD'))
GOLD_DO          <- Master_table_LONG_means %>% dplyr::filter(variable == 'do_clean' & Site %in% c('GOLD'))
GOLD_CHLA        <- Master_table_LONG_means %>% dplyr::filter(variable == 'chla_clean' & Site %in% c('GOLD'))

#write.csv(GOLD_Temperature, "output/Sonde Master Files/GOLD_Temperature_Sonde.csv")
...
```{r}
#Overlay of temp by year - GOLD
gold_temp<- ggplot(GOLD_Monthly_Temperature, aes(x=Month, y=Monthly_mean, group = Year, fill = factor(Year)))+
  geom_line(linewidth=0.75) +
  geom_ribbon(aes(ymax = Monthly_mean+sd, ymin = Monthly_mean-sd), alpha=0.15)+
  labs(x="Month", y="Temperature", colour="Year") +
  theme_classic()
