...
last updated 08/24/26 by M. Kachmar
- QAQC GOLD
...
file_paths <- list.files(path = "Field_Data_Recruitment/raw_data/spat_count",
...
data_all <- lapply(file_paths, function(file_path) {
  read_csv(file_path, show_col_types = FALSE) %>%
    mutate(
      date_collected = as.Date(parse_date_time(
        as.character(date_collected),
        orders = c("Ymd", "mdY")
      )),
      date_deployed = as.Date(parse_date_time(
        as.character(date_deployed),
        orders = c("Ymd", "mdY")
      ))
    )
}) %>%
...
Avg_spat_stringer <- Avg_spat_stringer %>% mutate(Month = month(date_collected))
...
#2025
spat_2025<- Avg_spat_stringer%>% filter(Year == 2025)
ggplot(data=spat_2025, aes( y=Count_1 )) +
  geom_boxplot()+  
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Mean Recruitment 2025", x ="Site", y = "Mean Recruitment (spat/shell)")+
  facet_wrap(~Site)

#Counts graphs - all sites, all years
Avg_spat_graph <- Avg_spat_stringer %>% filter(!Site == "LAUR", !Year=="2023")

ggplot(Avg_spat_graph, aes(x=Month,y=Count_1, fill = Site))+
  geom_bar(stat = "identity", position = "dodge")+
  labs(title = "Spat Counts - All Sites 2024-2025",
       x = "Month", y = "Count")+
...
#Mean spat per shell per replicate
GOLD_total_spat <- df_GOLD %>%
  dplyr::group_by( date_collected, Replicate) %>%
  dplyr::summarize(sum_count_1 = sum(Count_1),sum_count_2 = sum(Count_2))
GOLD_total_spat

GOLD_spat_avg <- GOLD_total_spat %>%
  dplyr::group_by(Replicate, date_collected)%>%
 dplyr::summarize(Mean_1 = mean(sum_count_1), Mean_2 = mean(sum_count_2))
GOLD_spat_avg
...
```{r}
#Spat length measurements

file_paths <- list.files(path = "Field_Data_Recruitment/raw_data/spat_length",
                          pattern = "*.csv", full.names = TRUE)


spat_length <- list.files(path = "Field_Data_Recruitment/raw_data/spat_length",                           # Identify all CSV files
 pattern = "*.csv", full.names = TRUE) %>% 
lapply(read_csv) %>%   # Store all files in list
  bind_rows%>%
  mutate(date_collected = as.Date(date_collected,format = "%m/%d/%Y"))# Combine data sets into one data set 

#View(data_all)

spat_length_all <- spat_length %>% filter(!is.na(date_collected))

spat_length_sum <- summarySE(spat_length_all, measurevar="shell_height", groupvars=c("Site", "date_collected"))
