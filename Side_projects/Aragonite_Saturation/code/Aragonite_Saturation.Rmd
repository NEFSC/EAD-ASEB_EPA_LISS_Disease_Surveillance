...
```{r}
Aragonite_master <- read.csv("raw_data/CO2sys_Water_Quality_Data.csv", check.names=FALSE) #%>% dplyr::select(-1) # check.names = F allows with col names with spaces (opthwise fills with a '.') 

Aragonite_master <- rename(Aragonite_master, c("date_collected"="Date collected","Site"="Location", "Aragonite_Sat"="WAr out", "Total CO2 (ppm)"="xCO2 out (dry at 1 atm) (ppm)", "Temp(C)_Field"="temp(C) of field"))
...
Aragonite_master_long<- Aragonite_master %>%  
                      reshape2::melt(id.vars = c("date_collected", "Site","month","year"),
                                    measure.vars = c("Salinity","Temp(C)_Field","pH out","pCO2 out (matm)","HCO3 out (mmol/kgSW)","CO3 out (mmol/kgSW)","CO2 out (mmol/kgSW)","Aragonite_Sat","Total CO2 (ppm)"))
...
Mean_aragonite_sat_all <- ggplot(data=Aragonite_master_long %>% 
  ...
  scale_y_continuous(limits = c(0,2), breaks = seq(0, 2, by = 0.2))+ facet_wrap(.~year, nrow=1 , ncol=4)+
...
pdf(paste0(path = "C:/Users/sarina.dery/Documents/GitHub/EAD-ASEB_EPA_LISS_Disease_Surveillance/Side_projects/Aragonite_Saturation/outputs/Mean_aragonite_sat_all.pdf"), height = 8, width = 12)
