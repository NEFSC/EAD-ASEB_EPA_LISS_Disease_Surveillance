...
---
Last updated 10/28/24 by M.Kachmar
...
ggplot(data=growth_summary, aes(x=Month, y=Shell_length, color=Site)) +
                 geom_point(position = position_dodge(width = 0.3))+
                  geom_line()+ #added position_dodge to be able to see the first points since they are the same
                  geom_errorbar(aes(ymin=Shell_length-se, ymax=Shell_length+se), width=.2, position=position_dodge(0.3)) +  
...
CT_sonde<- read.csv("C:/Users/mariah.kachmar/documents/Github/EAD-ASEB_EPA_LISS_Disease_Surveillance/Sonde_Data/output/Sonde_monthly_means_CT.csv")%>% mutate(
    year = year(year_month),
    Month = month(year_month, label = TRUE, abbr = FALSE)) %>% filter(`year_month` > '2024-05-01' & `year_month` < '2024-11-01' ) #filter what date ranges are needed
CT_sonde
...
lme_CHLA <- lmer(mean ~ Site + (1 | Month), data = CHLA_jg) #Yes
Anova(lme_CHLA)
...
lme_ph_jg <- lmer(Shell_length ~ mean + Site +  (1 | Month), data = pH_merged)
Anova(lme_ph_jg)

lme_salinity_jg <- lmer(Shell_length ~ mean + Site + Month + (1 | Bag_number), data = salinity_merged)
Anova(lme_salinity_jg)

lme_temp_jg <- lmer(Shell_length ~ mean + Site + Month + (1 | Bag_number), data = temperature_merged)
Anova(lme_temp_jg)

lme_CHLA_jg <- lmer(Shell_length ~ mean + Site + Month + (1 | Bag_number), data = CHLA_merged)
Anova(lme_CHLA_jg)

lme_DO_jg <- lmer(Shell_length ~ mean + Site + Month + (1 | Bag_number), data = DO_merged)
Anova(lme_DO_jg)
...
Surv_CT <- Surv(time = data_all$Month, event = data_all$Survival) #create your survival object using surv()
