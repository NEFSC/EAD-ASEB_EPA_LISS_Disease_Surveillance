...
write.csv(growth_summary, "Side_projects/Juvenile_growth/outputs\\juvenilegrowth_summary.csv", row.names=FALSE)  #creating/printing csv file of all raw data merged together
print(growth_summary)


# TOTAgrowth_summary# TOTAL GROWTH RATE - growth rate across the entire sample period
...
#Lets make some plots of this data

CT_sonde_filtered <- CT_sonde %>%
  filter(variable %in% c("Temperature", "Salinity", "pH", "RDO.Concentration", "Chlorophyll.a.Fluorescence"))

Environmental.data.plot <-ggplot()+
              geom_line(data=CT_sonde_filtered%>% filter(Site == "FENC"), aes(x=`Month`, 
    ...
    geom_errorbar(data = CT_sonde_filtered %>% filter(Site == "FENC"),
                aes(x = Month, ymin = mean - se, ymax = mean + se, color = Site), width = 0.2)+
  
  geom_line(data=CT_sonde_filtered%>% filter(Site == "ASHC"), aes(x=`Month`, #ASHC middle
                       y=mean, color = Site))+
  geom_errorbar(data = CT_sonde_filtered %>% filter(Site == "ASHC"),
                aes(x = Month, ymin = mean - se, ymax = mean + se, color = Site), width = 0.2) +
...
scale_color_manual(values=c(FENC = "darksalmon",ASHC = "cornflowerblue")) +
  ...
  facet_wrap(~variable, scales = "free_y", ncol =2, nrow=3) + labs(x = "Month", y = "Mean Variable")
...
#output of graph
pdf(paste0(path = "Side_projects/Juvenile_growth/outputs/" ,"Environmental.data.plot.pdf"), height = 7, width = 11) #printing a PDF within the path of the code, with specific dimensions of PDF
print(Environmental.data.plot) #specify the graph
dev.off()
...
#R.sq >0.7 very good, 0.5-0.7 moderate (acceptable), <0.5 Weak (model doesnt explain much variation); Biological/ecological data: R² ~0.5 is often good due to natural variability.
# 
```{r}
#Is the monthly means of each environmental parameter different between sites?
...
####What if we added month because the above models are not considering time.
gam_ph_jg <- gam(Shell_length ~ s(mean) + Site + Site:mean + Month:mean, data = pH_merged)
summary(gam_ph_jg)

gam_ph_jg_t <- gam(Shell_length ~ te(mean, Month) + Site + Site:mean, data = pH_merged)
summary(gam_ph_jg_t)
#te() - smooth function option, allows the effect of mean on shell length to vary smoothly over month (time). 
#shows us that fence creek on average has higher shell height compared to ash creek (reference site) and is statistically significant
# shows that mean ph and oyster shell length varies over time and is highly significant - pH may have a stronger positive effect on shell growth in some months and not others. 

#so what months pH has the most effect? Post hoc...emmeans() , derivatives()
library(emmeans)

# Define levels to test
posthoc_grid <- list(
  Month = unique(pH_merged$Month),
  mean = mean(pH_merged$mean)  # Fix pH to mean value
)

pH_merged_ASHC <- pH_merged %>% filter (Site == "ASHC")
View(pH_merged_ASHC)

gam_ph_jg_t <- gam(Shell_length ~ te(mean, Month), data = pH_merged_ASHC)
summary(gam_ph_jg_t)

em_month <- emmeans(gam_ph_jg_t, "Month")
summary(em_month)
...
gam_salinity_jg_t <- gam(Shell_length ~ te(mean,Month) + Site+ Site:mean, data = salinity_merged)
summary(gam_salinity_jg_t)

# Define levels to test
posthoc_grid_s <- list(
  Month = unique(salinity_merged$Month),
  mean = mean(salinity_merged$mean)  # Fix pH to mean value
)

em_month_s <- emmeans(gam_salinity_jg_t, ~ Month | Site, at = posthoc_grid_s)
summary(em_month_s)

pairs(em_month_s)

df <- as.data.frame(em_month_s)

S_emmeans_plot<-ggplot(df, aes(x = Month, y = emmean, color = Site, group = Site)) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL, fill = Site), alpha = 0.2, color = NA) +
  labs(y = "Salinity (PSU)", title = "Monthly Changes in Salinity by Site") +
  theme_minimal()

####

gam_temp_jg_t <- gam(Shell_length ~ te(mean,Month) + Site+ Site:mean, data = temperature_merged)
summary(gam_temp_jg_t)

# Define levels to test
posthoc_grid_t <- list(
  Month = unique(temperature_merged$Month),
  mean = mean(temperature_merged$mean)  # Fix pH to mean value
)

em_month_t <- emmeans(gam_temp_jg_t, ~ Month | Site, at = posthoc_grid_t)
summary(em_month_t)

pairs(em_month_t)
####

gam_CHLA_jg_t <- gam(Shell_length ~ te(mean,Month) + Site+ Site:mean, data = CHLA_merged)
summary(gam_CHLA_jg_t)

# Define levels to test
posthoc_grid_c <- list(
  Month = unique(CHLA_merged$Month),
  mean = mean(CHLA_merged$mean)  # Fix pH to mean value
)

em_month_c <- emmeans(gam_CHLA_jg_t, ~ Month | Site, at = posthoc_grid_c)
summary(em_month_c)
...
###
gam_DO_jg_t <- gam(Shell_length ~ te(mean,Month) + Site+ Site:mean, data = DO_merged)
summary(gam_DO_jg_t)

# Define levels to test
posthoc_grid_d <- list(
  Month = unique(DO_merged$Month),
  mean = mean(DO_merged$mean)  # Fix pH to mean value
)

em_month_d <- emmeans(gam_DO_jg_t, ~ Month | Site, at = posthoc_grid_d)
summary(em_month_d)
...
#add random effects to see if results vary

fit.coxph_frailty <- coxph(Surv_CT ~ Site + frailty(Bag_number), data = data_all) #added bag number for replication using the frailty function - incorporating a random effect to the hazard model
fit.coxph_frailty

hazardratio.coxph.frailty   <- ggforest(fit.coxph_frailty, data = data_all) #not allowing me to create our plot. 

#lets try coxme()
library(coxme)
library(gtsummary)

coxme.test <- coxme(Surv_CT ~ Site + (1| Bag_number), data = data_all)
coxme.test

capture.output(coxme.test,
               file="Side_projects/Juvenile_growth/outputs/_Survival_CoxRegMixed.doc")

coxme.test.table   <- coxme(Surv_CT ~ Site + (1| Bag_number), data = data_all) %>%  
                             tbl_regression(exp = TRUE) 

write.csv(as.data.frame(coxme.test.table),file="Side_projects/Juvenile_growth/outputs/Jg_Survival_CoxRegMixed.csv")
