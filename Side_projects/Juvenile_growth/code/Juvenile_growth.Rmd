...
---
Last updated 11/7/24 by M.Kachmar
- Growth rate and glms
...
ggplot(data=growth_summary, aes(x=Month, y=monthly_growth_rate, color=Site, group = Site)) +
        ...
        geom_errorbar(aes(ymin=monthly_growth_rate-se, ymax=monthly_growth_rate+se), width=.2, position=position_dodge(0)) +  
  # scale_color_manual(values=c("green4", "darkorange1"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs( x ="Month", y = "Growth Rate (mm/month")+ylim(0,20) #+ facet_wrap(~Site, nrow = 2)+

ggplot(data=growth_summary, aes(x=Month, y=monthly_growth_rate, fill=Site)) +
...
glm_ph <- glmmTMB(mean ~ Month + (1 | Site), data = pH_jg ) #This is significant but the kruskal wallace is not, should I do this instead? this will take into account the variability across sites as well. 
Anova(glm_ph)
summary(glm_ph)
...
#Kruskal-Wallis tests by month (P-value sig. 0.0125 for four tests)
#Does environmental data vary monthly?
kruskal.test(mean ~ Month, pH_jg)
kruskal.test(mean ~ Month, Salinity_jg)
kruskal.test(mean ~ Month, Temperature_jg)
kruskal.test(mean ~ Month, CHLA_jg)
kruskal.test(mean ~ Month, DO_jg)

#they do not vary monthly
...
glm_size <- glmmTMB(Shell_length ~ Site + (1|Month) +(1|Bag_number), data = growth_master)
Anova(glm_size)
# Yes 
...
kruskal.test(Shell_length ~ Site, data = Nov)

#all of these are significant so which month is the most significant? August has the smallest p value (<0.0001).
...
summary(glm_ph_jg)
#Shell length is impacted by salinity. The interaction shows that relationship between salinity and shell length varies between sites.  
...
#So what did we learn from these models? 
  #CHLA is different at each site - one has more food availability than the other. All environmental conditions interact to influence the differences in shell heigh at each site- aka the combine environmental factors create differences between sites. pH does not impact shell length alone and its relationship with shell length is dependent on site.  temperature is independent of site (so the same trend is happening at both) with positive relationships with shell length - as temperature increases so does shell length/growth. 
...
AIC(glm_be, glm_ALL) #glm_ALL has a slightly lower AIC so it is a better fit model
...
#Hypothesis: Shell length can be explained by mean environmental conditions, site differences, and random variation across months and bags. 
glm_gr_ALL <- glmmTMB(monthly_growth_rate ~ Site + mean_Salinity + mean_RDO.Concentration + mean_Chlorophyll.a.Fluorescence + mean_pH + mean_Temperature + (1|Month), data = gr_merged )
Anova(glm_gr_ALL)
summary(glm_gr_ALL)
...
AIC(glm_be, glm_ALL) #glm_ALL has a lower AIC so it is a better fit model
