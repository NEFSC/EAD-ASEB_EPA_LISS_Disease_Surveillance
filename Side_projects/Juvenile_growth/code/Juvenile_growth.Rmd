...
---
Last updated kennedy testing push again
- Working on Katie's suggestions
...
#Is shell length different between sites and months?
gam_size <- gam(Shell_length ~ Site + Month , data = growth_master)
summary(gam_size)
# Yes 
...
gam_ph_jg <- gam(Shell_length ~ s(mean) + Site, data = pH_merged)
summary(gam_ph_jg)


gam_salinity_jg <- gam(Shell_length ~ s(mean) + Site, data = salinity_merged)
summary(gam_salinity_jg)

gam_temp_jg <- gam(Shell_length ~ s(mean) + Site, data = temperature_merged)
summary(gam_temp_jg)

gam_CHLA_jg <- gam(Shell_length ~ s(mean) + Site, data = CHLA_merged)
summary(gam_CHLA_jg)

gam_DO_jg <- gam(Shell_length ~ s(mean) + Site, data = DO_merged)
summary(gam_DO_jg)
...
#Hypothesis: Shell length can be explained by mean environmental conditions, site differences, and random variation across months and bags. 
gam_ALL <- glmmTMB(Shell_length ~ Site + mean_Salinity + mean_RDO.Concentration + mean_Chlorophyll.a.Fluorescence + mean_pH + mean_Temperature, data = jg_merged )
summary(gam_ALL)

gam_ALL_smooth <- gam(Shell_length ~ Site + s(mean_Salinity) + s(mean_RDO.Concentration) + s(mean_Chlorophyll.a.Fluorescence) + s(mean_pH) +s(mean_Temperature), data = jg_merged ) 
summary(gam_ALL_smooth)

AIC(gam_ALL,gam_ALL_smooth) #AIC lower for gam smooth


################################################ Growth Rate ###############################################

#growth rate different between sites and months?
gam_growth <- gam(monthly_growth_rate ~  Site + Month, data=growth_summary) 
summary(gam_growth)
...
gam_ph_gr <- gam(monthly_growth_rate ~ mean + Site , data = pH_gr)
summary(gam_ph_gr)

gam_salinity_gr <- gam(monthly_growth_rate ~ mean + Site, data = salinity_gr)
summary(gam_Salinity_gr)

gam_temp_gr <- gam(monthly_growth_rate ~ mean + Site , data = temperature_gr)
summary(gam_temp_gr)

gam_CHLA_gr <- gam(monthly_growth_rate ~ mean + Site , data = CHLA_gr) #CHLA significant
summary(gam_CHLA_gr)

gam_DO_gr <- gam(monthly_growth_rate ~ mean + Site  , data = DO_gr)
summary(gam_DO_gr)
...
gam_gr_smooth <- gam(monthly_growth_rate ~ Site + s(mean_Salinity) + s(mean_RDO.Concentration) + s(mean_Chlorophyll.a.Fluorescence)
