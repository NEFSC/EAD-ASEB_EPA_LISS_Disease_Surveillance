...
for (m in 1:nrow(file.names.qpcr)) { # this is your loop, for each row in th filenames tbale, do the followin!
  ...
  ind      <- grep("Well",D) # the row number containing the start of the data...in this case "Well" however there is another line that says "well group" and it is reading that. need to figure out how fix that. 
  ...
  raw      <- read_csv(raw_qpcr_rootdir,skip=ind-1,col_types = cols())
