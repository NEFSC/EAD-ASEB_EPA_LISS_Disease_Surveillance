...
---
Last updated 6/27/25 by K. Lenderman
  - Accuracy 82.6%
  - Master file and Completeness file are up to date.
  - 0625FENC was only 3.3% reread
...
FENC_proportion_year <- df_intensity_proportions%>% filter(site == "FENC")

FENC_proportion_yearly<- ggplot(data=FENC_proportion_year, aes(x=month, y= Proportion, fill=intensity_bin)) +
  geom_bar(width = .5, stat="identity", position = "fill",colour = "black")+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(fill ="Dermo Intensity")+
