...
Updated 1/15/2025 by K.Lenderman
 - data check
 - added new code for 2026 data
 
 2/12/26 by M.Kachmar
 - added some preliminary statistics for body condition 
...
```{r}
#Winter sampling 
# winter_bcs <- rbind(df_BCS_proportions2024, df_BCS_proportions2025, df_BCS_proportions2026)
# 
...
```{r}
# Proportion per month of intensity scores at three sites during 2023 - 2026
...
#Is BC significantly different across sites?
#Is BC significantly different over time? 
kruskal.test(condition_score ~ site ,data_all)
kruskal.test(condition_score ~ date_collected ,data_all)

#Is BC different between sites independent of date?
#is BC different between dates independent of site?
#Does BC over time differ at each site? (interaction *)
glm_bc <- glmmTMB(condition_score ~ site * date_collected, data_all)
summary(glm_bc)
Anova(glm_bc)


####### Lets look just at CT sites ########
CT_bc_all <- filter(data_all, !site  %in% c("GOLD", "LAUR"))
CT_bc_monthlymean <- filter(mean_body_condition, !site  %in% c("GOLD", "LAUR"))

#Is BC significantly different across sites?
#Is BC significantly different over time? 
kruskal.test(condition_score ~ site ,CT_bc_all)
kruskal.test(condition_score ~ date_collected ,CT_bc_all)

#Is BC different between sites independent of date?
#is BC different between dates independent of site?
#Does BC over time differ at each site? (interaction *)
glm_bc_CT <- glmmTMB(condition_score ~ site * date_collected, CT_bc_all)
summary(glm_bc_CT)
Anova(glm_bc_CT)
...
```{r}
#Body condition scores proportion graph (nested month and year) for ASHC, FENC, GOLD
df_intensity_summary_1<- data_all %>%
  group_by(site, date_collected, month,year, condition_score) %>%
...
df_intensity_summary_1$condition_score <- factor(df_intensity_summary_1$condition_score, levels = c("1","2","3","4","5","6","7","8","9"))

FENC_ratio_intensity <- ggplot(data=filter(df_intensity_summary_1,site=="FENC"), aes(x=year, y= Proportion, fill=condition_score)) +
  geom_bar(width = 15, stat="identity", position = "fill")+
  ggh4x::facet_nested(~year+month, scales = "free", switch = "x")+
  labs(x="", title = "Fence Creek", fill = "Body Condition Score")+
  theme_minimal()+
  ...
  theme(panel.grid = element_line(colour = "white"), axis.text.x = element_blank(), axis.text = element_text(size= 13), axis.title = element_text(size = 15), strip.text = element_text(size = 13), legend.text = element_text(size = 13), legend.title = element_text(size = 15), title = element_text(size = 16), axis.title.y = element_text(vjust = 2.5),axis.title.x = element_text(vjust = -1))+ scale_fill_brewer(palette = "Blues", direction = -1)+theme(plot.margin = unit(c(0.75, 0, -0.75, 1), "cm"))

ASHC_ratio_intensity <- ggplot(data=filter(df_intensity_summary_1,site=="ASHC"), aes(x=year, y= Proportion, fill=condition_score)) +
  geom_bar(width = 15, stat="identity", position = "fill")+
  ggh4x::facet_nested(~year+month, scales = "free", switch = "x")+
  labs(x="", title = "Ash Creek", fill = "Body Condition Score")+
  theme_minimal()+
  theme(strip.text = element_text(face = "bold"), ggh4x.facet.nestline=element_line(colour = "black"))+scale_x_continuous(breaks = 1:12, labels = month.abb)+
  theme(panel.grid = element_line(colour = "white"), axis.text.x = element_blank(), axis.text = element_text(size= 13), axis.title = element_text(size = 15), strip.text = element_text(size = 13), legend.text = element_text(size = 13), legend.title = element_text(size = 15), title = element_text(size = 16), axis.title.y = element_text(vjust = 2.5),axis.title.x = element_text(vjust = -1))+ scale_fill_brewer(palette = "Blues", direction = -1)+theme(plot.margin = unit(c(0.75, 0, -0.75, 1), "cm"))

GOLD_ratio_intensity <- ggplot(data=filter(df_intensity_summary_1,site=="GOLD"), aes(x=year, y= Proportion, fill=condition_score)) +
  geom_bar(width = 15, stat="identity", position = "fill")+
  ggh4x::facet_nested(~year+month, scales = "free", switch = "x")+
  labs(x="", title = "Gold Star", fill = "Body Condition Score")+
  theme_minimal()+
  theme(strip.text = element_text(face = "bold"), ggh4x.facet.nestline=element_line(colour = "black"))+scale_x_continuous(breaks = 1:12, labels = month.abb)+
  theme(panel.grid = element_line(colour = "white"), axis.text.x = element_blank(), axis.text = element_text(size= 13), axis.title = element_text(size = 15), strip.text = element_text(size = 13), legend.text = element_text(size = 13), legend.title = element_text(size = 15), title = element_text(size = 16), axis.title.y = element_text(vjust = 2.5),axis.title.x = element_text(vjust = -1))+ scale_fill_brewer(palette = "Blues", direction = -1)+theme(plot.margin = unit(c(0.75, 0, -0.75, 1), "cm"))

bcs_all <- ggarrange(ASHC_ratio_intensity, FENC_ratio_intensity, GOLD_ratio_intensity, ncol=1)
bcs_all
```

```{r}
dotplot(data = df_GOLD, aes(x=month, y=condition_score))
```
