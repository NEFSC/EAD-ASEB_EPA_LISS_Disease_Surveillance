...
Last updated 9/3/26 by M.Kachmar
- Fence creek fall 2026
...
#                                     labels = c("2023-10-03","2023-10-05","2023-10-16", "2023-10-17" ))
  ...
  dplyr::mutate(month = month(Date_collected)) #lets add a month column to our data to better organize later on for graphing etc. 
...
height_all <- dplyr::select(height_all, -c("month"))
...
height_live <- dplyr::select(height_all, 'Field ID', 'Date_collected', 'Site', 'State', 'Quadrat','Height_live', 'Season')
...
height_box <- dplyr::select(height_all, 'Field ID', 'Date_collected', 'Site', 'State', 'Quadrat','Height_box', 'Season')
...
height_gaper <- dplyr::select(height_all, 'Field ID', 'Date_collected', 'Site', 'State', 'Quadrat','Height_gaper', 'Season')
...
#reorder columns
Height_all <- Height_all %>% dplyr::select("Date_collected", "Site", "State", "Quadrat", "Height", "Survival_status", "Season")
...
write_csv(Height_master,  "Master_files/height_all_data.csv") #row.names=FALSE)
...
#Distribution Dead 
ASHC_Distribution_Dead<- height_ASHC_Dead %>% 
  dplyr::filter(Year == "2026" ) %>%
  ggplot(aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
...
FENC_Distribution_Live<- height_FENC_Live %>% 
 ...
 facet_wrap(~ factor(Season, levels = c("Spring", "Fall")) + Year,  scales = "fixed", nrow=2)
...
#Distribution Dead
FENC_Distribution_Dead<- height_FENC_Dead %>% 
  dplyr::filter(Year == "2026")%>%
  ggplot( aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
...
ggplot(data=df_quadrat_FENC, aes(x= survival_status, y = Count, fill = survival_status)) +
  ...
  facet_wrap(~ factor(Season, levels = c("Spring", "Fall")) + Date_collected, scales = "fixed")
...
```{r}
df_quadrat_GOLD <- density%>%
  filter(Site == "GOLD")%>%
  mutate(year = year(Date_collected))
df_quadrat_GOLD

#for 2023 - 2025 Quadrat size is 50 x50cm (.25 m^2). To calculate density (oysters per m^2) multiply number of oysters counted X 4 to = 1 meter
#for 2026, we chose to use 25 x25cm (.0625 m^2). To calculate density (oysters per m^2) multiply number of oysters counted X 16

df_density_GOLD <- df_quadrat_GOLD %>%
  dplyr::group_by(Site, Season, year, Date_collected, Quadrat, survival_status) %>% 
   dplyr::filter(year != '2026') %>% #removing 2026 to calculate that separately
  dplyr::summarize(Density = sum(Count)*4)
df_density_GOLD

df_density_GOLD_2026 <- df_quadrat_GOLD %>%
  dplyr::group_by(Site, Season, year, Date_collected, Quadrat, survival_status) %>% 
   dplyr::filter(year == '2026') %>% 
  dplyr::summarize(Density = sum(Count)*16)
df_density_GOLD_2026

df_density_GOLD_all <- rbind(df_density_GOLD,df_density_GOLD_2026)

#Mean of Oysters per m^2 across all quadrats
mean_total_oysters_GOLD <- df_density_GOLD_all %>%
