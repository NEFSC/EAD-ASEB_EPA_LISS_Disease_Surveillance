...
ggplot(data=mean_total_oysters_all, aes(x= survival_status, y = Total_Density, fill = survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Mean oyster/m^2 Density ", x ="Survival status", y= "Density (# oyster / meter squared)")+
  facet_wrap(~ Season + Date_collected + Site, scales = "free", nrow=3) #+
