...
updated 9/3/2026 M.Kachmar
- input fall 2026 fence creek
...
data_all <- dplyr::select(data_all, -c("month"))
...
bed_all <- dplyr::select(data_all, -c("Field ID", "Quadrat_size", "Percent_coverage", "Live_count",
...
#reordering columns
bed_all <- bed_all %>% dplyr::select("Date_collected", "Site", "State", "Bed_height_cm", "Season")
...
#removing unnecessary columns
bedheight_june2023 <- dplyr::select(bedheight_june2023, -c("BedHeight_Notes", "TopMark", "BottomMark", "Measure_direction", "Bed_Section"))
...
#reordering columns
bedheight_june2023 <- bedheight_june2023 %>% dplyr::select("Date_collected", "Site", "State", "Bed_height_cm", "Season")
```
...
#removing unnecessary columns
rugosity_all <- dplyr::select(data_all, -c("Field ID", "Quadrat_size", "Percent_coverage", "Live_count",
                                  "Box_count", "Gaper_count","Bed_height", "Quadrat"))

rugosity_all <- rugosity_all %>% dplyr::select(where(~ !all(is.na(.))))
rugosity_all <- rugosity_all%>% dplyr::filter(!is.na(Rugosity))
#head(rugosity_all)
```
...
#removing unnecessary columns
rugosity_june2023 <- dplyr::select(rugosity_june2023, -c("RUGOS_NUM","ChainType", "Bed_Section", "Zone", "LengthContoured_cm", "LengthStraightline_cm","Notes"))
...
#reordering columns
rugosity_june2023 <- rugosity_june2023 %>% dplyr::select("Date_collected", "Site", "State", "Rugosity", "Season")
```
...
rugosity_summary <- rugosity_summary %>% filter(Site != "LAUR")

#box plot comparing the sites
ggplot(rugosity_summary, aes(x = Date_collected, y = Rugosity, fill = Site)) +
  geom_boxplot(alpha = 0.7) +
  geom_jitter(aes(color = Site, width = 0.15, alpha = 0.6, size = 2, show.lengend = FALSE)) +
  theme_classic() +
  labs(
    title = "Oyster Bed Rugosity ",
    x = "Date",
    y = "Rugosity Index"
  ) +
  scale_fill_manual(values = c("darkgreen", "steelblue", "darkred"))+
  scale_color_manual(values= c("darkgreen", "steelblue", "darkred") )+
  theme(plot.title = element_text(size= 18, face = "bold", hjust= 0.5), axis.title = element_text(size = 11), legend.title = element_blank())+
  facet_wrap(~Site)
  


#scatterplot gold star rugosity by restoration age (year)

rugosity_GOLD <- rugosity_summary %>% filter(Site == "GOLD")

ggplot(rugosity_GOLD, aes(x = Date_collected, y = Rugosity)) +
  geom_point(color = "orange", alpha = 0.6, size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "black", fill = "gray") +
  theme_classic() +
  labs(
    title = "Oyster Reef Rugosity Over Time",
    x = "Age of Restored Reef (Years)",
    y = "Rugosity Index"
  )
...
#removing unnecessary columns
percov_june2023 <- dplyr::select(percov_june2023, -c("QUADRAT_NUM", "Quadrat_Type", "Quadrat_Zone", "Light_Zone", "Bed_Section", "LiveCount_Total", "GaperCount_Total", "BoxCount_Total", "Subsample_Measured", "Quadrat_Notes"))
...
#reordering columns
percov_june2023 <- percov_june2023 %>% dplyr::select("Date_collected", "Site", "State", "Percent_coverage", "Season")
```
...
#removing unnecessary columns
percent_coverage <- dplyr::select(data_all, -c("Field ID", "Quadrat_size", "Rugosity", "Bed_height", "Quadrat", "Live_count", "Box_count", "Gaper_count"))

percent_coverage <- percent_coverage%>% dplyr::filter(!is.na(Percent_coverage))
...
percent_coverage <- percent_coverage %>%
  dplyr::mutate(Percent_coverage = dplyr::recode(Percent_coverage, 
                                    "15" = "0-24", "20" = "0-24", "25" = "25-49", "30" = "25-49", "40" = "25-49", "50"= "50-74", "80"= "75-99", "90"= "75-99", "100"= "100"))

#reordering columns
percent_coverage <- percent_coverage %>% dplyr::select("Date_collected", "Site", "State", "Percent_coverage", "Season")
...
#data Fall 2023 - present and clean it up ; Removing unnecessary columns as we are just focusing on density information, removing any NAs, creating a month column
...
quadratdensity <- quadratdensity %>%  select(Date_collected, Site, Quadrat, Live_count, Box_count, Gaper_count, Season)
...
density_live_sum <- density_live_sum %>%
  dplyr::mutate(Count = Live_count) %>%  dplyr::mutate(survival_status = "live") %>%
  dplyr::select(-Live_count)
density_live_sum
...
density_box_sum <- density_box_sum %>%
  dplyr::mutate(Count = Box_count) %>% dplyr::mutate(survival_status = "box") %>%
  dplyr::select(-Box_count)
density_box_sum
...
density_gaper_sum <- density_gaper_sum %>%
  dplyr::mutate(Count = Gaper_count) %>% dplyr::mutate(survival_status = "gaper") %>%
  dplyr::select(-Gaper_count)
density_gaper_sum
...
#calculate completeness as 100*N/4, where 4 is the desired number of quadrats per site. 2023 surveys had 12 quadrates measured. 
  densitysummary <- density_summary %>%
    dplyr::mutate(PercentComplete_Quad = ((N/4)*100))
densitysummary
...
density <- quadratdensity_summary %>%
  ...
  dplyr::mutate(survival_status = case_when(
...
mean_total_oysters_all <- df_density_all %>%
 ...
 dplyr::summarize(Total_Density = mean(Density), SD = sd(Density, na.rm = TRUE), n = n(), SE = SD / sqrt(n), .groups = "drop")%>%dplyr::mutate(Year = year(Date_collected)) 
...
ggplot(data=mean_total_oysters_all %>% filter(Site != "LAUR"), aes(x= survival_status, y = Total_Density, fill = survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Density - mean oysters " ~ m^{-2} ~ " ", x ="Survival status", y= expression("Density (mean oysters " ~ m^{-2} ~ ")"))+
  facet_grid(~ Site + Season ~ Year, scales = "free_x", space="free_x")+
 #geom_text(aes(label=Total_Density), vjust=-0.3, size = 3.5)+
geom_errorbar(aes(ymin = Total_Density - SE,
                  ymax = Total_Density + SE),
              width = 0.2) 

#use to filter out specific sites
ggplot(data=mean_total_oysters_all %>% filter(Site == "ASHC", Season == "Spring"), aes(x= survival_status, y = Total_Density, fill = survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Density - mean oysters " ~ m^{-2} ~ " ", x ="Survival status", y= expression("Density (mean oysters " ~ m^{-2} ~ ")"))+
