...
updated 5/14/2026 M.Kachmar
- input spring 2026 data for Fence Creek
...
ggplot(data=mean_total_oysters_all %>% filter(Site == "FENC", Season == "Spring"), aes(x= survival_status, y = Total_Density, fill = survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.title = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Density - mean oysters " ~ m^{-2} ~ " ", x ="Survival status", y= expression("Density (mean oysters " ~ m^{-2} ~ ")"))+
