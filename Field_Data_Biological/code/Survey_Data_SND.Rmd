...
# 1. Process data and calculate proportions properly WITHIN each site
...
ggplot(Spring_shellheight_proportions2026, aes(x = size_class_bin, y = Proportion)) + 
  ...
  labs( 
    title = "Spring Shell Height Distribution (2026)", 
    x = "Size Class Bin (mm)", 
    y = "Proportion" 
  ) + 
...
# 2. Generate the faceted plot
ggplot(Fall_shellheight_proportions2025, aes(x = size_class_bin, y = Proportion)) + 
  geom_col(fill = "steelblue", color = "black", width = 0.8) + 
  scale_y_continuous(labels = scales::percent_format()) + 
  scale_x_discrete(drop = FALSE) + 
  facet_wrap(~ Site) + # Fix 1: Moved facet inside the ggplot chain with a "+"
  labs( 
    title = "Fall Shell Height Distribution (2025)", 
    x = "Size Class Bin (mm)", 
    y = "Proportion" 
  ) + 
  theme_minimal() + #applies grid lines
