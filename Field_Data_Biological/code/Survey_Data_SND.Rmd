...
ggplot(data=mean_total_oysters_all%>%filter(Season=="Spring")%>%filter(Site!="LAUR")%>%filter(Site!="GOLD"), aes(x= survival_status, y = Total_Density, fill = survival_status)) +
  ...
  facet_wrap(~ Site + Date_collected , scales = "free_x", ncol=3)  #+geom_text(aes(label=Total_Density), vjust=-0.3, size = 3.5)
...
shellsize<- dplyr::select( shellsize, -c("QuadSpec_NUM", "Year", "Month", "Day", "Length_mm", "Width_mm", "Specimen_Notes")) 

#Changing site name and removing unneeded column
shellsize<- shellsize %>%
  dplyr::mutate(Site = dplyr::recode(Site_Name, "Ash Creek" = "ASHC", "Fence Creek" = "FENC")) %>% 
...
#reorder columns
shellsize <- shellsize %>% dplyr::select("Date_collected", "Site", "State", "Quadrat", "Height", "Survival_status", "Season")
...
ggplot(Height_master%>% dplyr::filter(Survival_status=="Live") %>% filter(Site!="LAUR")%>% filter(Site!="GOLD"), aes(x=Season, y=Height, fill=Site)) +
  ...
  labs(title="Shell Height Live", x ="Season", y = "Height (mm)")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
  facet_wrap(~Site +Year)

```
#Spring 2026 live data only
```{r}

#Isolate Live Data and drop unwanted sites
CT_shell_height_live <- Height_master %>% 
  dplyr::filter(Survival_status == "Live") %>% 
  dplyr::filter(Site != "LAUR") %>% 
  dplyr::filter(Site != "GOLD") 

#Process data and calculate proportions properly WITHIN each site
Spring_shellheight_percent2026_data <- CT_shell_height_live %>% 
  dplyr::filter(Year == "2026") %>% 
  dplyr::filter(Season == "Spring") %>% 
  dplyr::filter(!is.na(Height)) %>% 
  dplyr::mutate(
    size_class = as.numeric(Height), 
    size_class_bin = cut(
      size_class, 
      breaks = seq(0, 210, by = 10), 
      labels = paste0(seq(0, 200, by = 10), "-", seq(10, 210, by = 10)), 
      include.lowest = TRUE, 
      right = TRUE
    )
  ) %>% 
 group_by(Site, size_class_bin, Survival_status) %>% 
  dplyr::summarise(Count = n(), .groups = "drop") %>% # Drop all groups safely
  group_by(Site) %>%                                  # Regroup ONLY by Site
  mutate(Percentage = Count / sum(Count)) %>%        # Calculates % across the whole Site population
  ungroup() 

#Generate the faceted plot (using a unique plot variable name)
Spring_shellheight_percent2026_plot <- ggplot(
    Spring_shellheight_percent2026_data, 
    aes(x = size_class_bin, y = Percentage)
  ) + 
  ...
  facet_wrap(~ Site) + 
  labs(
    title = "Spring Survey Live Shell Height (2026)", 
    x = "Size Class Bin (mm)", 
    y = "Total Percentage"
  ) + 
  theme_minimal() + 
  theme(
    panel.grid.minor = element_blank(),                       # Keeps major grids but hides messy minor ones
...
#png("Field_Data_Biological/Output/Spring_Survey_Live_Shell_Height(2026).png", height = 6, width = 11, units = "in", res = 300)
print(Spring_shellheight_percent2026_plot)
#dev.off()

```

#Spring 2026 survival & size class - proportion of total site population

```{r}
CT_shell_height_all <- Height_master%>% dplyr:: filter(Site!="LAUR")%>% filter(Site!="GOLD")

#Data Wrangling & Distribution Calculation per Site
CT_Spring_Survey_2026 <- CT_shell_height_all %>%
  dplyr::filter(Year == "2026") %>%
  dplyr::filter(Season == "Spring") %>% 
  dplyr::filter(!is.na(Height)) %>% 
  dplyr::mutate(
  ...
  dplyr::group_by(Site, size_class_bin, Survival_status) %>% 
  dplyr::summarise(Count = n(), .groups = "drop") %>% 
  # Group strictly by Site so your proportions reflect the whole site population
  dplyr::group_by(Site) %>% 
  dplyr::mutate(Proportion = Count / sum(Count)) %>% 
  dplyr::ungroup() 

#Generate the Corrected Plot Layout
CT_Spring_Survey_2026_population_percentages <- ggplot(data = CT_Spring_Survey_2026, aes(x = size_class_bin, y = Proportion, fill = Survival_status)) + 
  # FIX: Changed position to "stack" to honor your calculated 'Proportion' values
  geom_bar(width = 0.8, stat = "identity", position = "stack", colour = "gray10") + 
  # ADDED: Formats the Y axis decimal numbers (0.10, 0.20) as clean percentages (10%, 20%)
  scale_y_continuous(labels = scales::percent_format()) +
  scale_x_discrete(drop = FALSE) + 
  labs(
    title = "CT Spring Survey 2026", 
    x = "Size Class Bin (mm)", 
...
#png("Field_Data_Biological/Output/CT_Spring_Survey_2026_population_percentages.png", height = 6, width = 11, units = "in", res = 300)
print(CT_Spring_Survey_2026_population_percentages)
#dev.off()

```

#Fall 2026 survival & size class - proportion of total site population

```{r}

#Data Wrangling & Distribution Calculation per Site
CT_Fall_Survey_2026 <- CT_shell_height_all %>%
  dplyr::filter(Year == "2026") %>%
...
#Generate the Corrected Plot Layout
CT_Fall_Survey_2026_population_percentages <- ggplot(data = CT_Fall_Survey_2026, aes(x = size_class_bin, y = Proportion, fill = Survival_status)) + 
  # FIX: Changed position to "stack" to honor your calculated 'Proportion' values
  geom_bar(width = 0.8, stat = "identity", position = "stack", colour = "gray10") + 
  # ADDED: Formats the Y axis decimal numbers (0.10, 0.20) as clean percentages (10%, 20%)
  scale_y_continuous(labels = scales::percent_format()) +
...
#png(("Field_Data_Biological/Output/CT_Fall_Survey_2026_population_percentages.png"), height = 6, width = 11, units = "in", res = 300)
print(CT_Fall_Survey_2026_population_percentages)
#dev.off()
```

```

#Fall 2025 live data only
```{r}
#Process data and calculate proportions properly WITHIN each site
Fall_shellheight_percent2025_data <- CT_shell_height_live %>% 
...
#Generate the faceted plot (using a unique plot variable name)
Fall_shellheight_percent2025_plot <- ggplot(
    Fall_shellheight_percent2025_data, 
    aes(x = size_class_bin, y = Percentage)
  ) + 
  geom_col(fill = "steelblue", color = "black", width = 0.8) + 
  scale_y_continuous(labels = scales::percent_format()) + 
  ...
  facet_wrap(~ Site) + 
  labs(
    title = "Fall Survey Live Shell Height (2025)", 
    x = "Size Class Bin (mm)", 
    y = "Total Percentage"
  ) + 
  theme_minimal() + 
...
#png("Field_Data_Biological/Output/Fall_Survey_Live_Shell_Height(2025).png", height = 6, width = 11, units = "in", res = 300)
print(Fall_shellheight_percent2025_plot)
#dev.off()

```


#Fall 2025 survival & size class - proportion of total site population

```{r}
#Data Wrangling & Distribution Calculation per Site
CT_Fall_Survey_2025 <- CT_shell_height_all %>%
  dplyr::filter(Year == "2025") %>%
  dplyr::filter(Season == "Fall") %>% 
  dplyr::filter(!is.na(Height)) %>% 
  dplyr::mutate(
...
#Generate the Corrected Plot Layout
CT_Fall_Survey_2025_population_percentages <- ggplot(data = CT_Fall_Survey_2025, aes(x = size_class_bin, y = Proportion, fill = Survival_status)) + 
  # FIX: Changed position to "stack" to honor your calculated 'Proportion' values
  geom_bar(width = 0.8, stat = "identity", position = "stack", colour = "gray10") + 
  # ADDED: Formats the Y axis decimal numbers (0.10, 0.20) as clean percentages (10%, 20%)
  scale_y_continuous(labels = scales::percent_format()) +
  scale_x_discrete(drop = FALSE) + 
  labs(
    title = "CT Fall Survey 2025", 
    x = "Size Class Bin (mm)", 
    y = "Proportion of Total Site Population"
  ) + 
  theme_minimal() + 
  theme(
    panel.grid.minor = element_blank(),
...
#png(("Field_Data_Biological/Output/CT_Fall_Survey_2025_population_percentages.png"), height = 6, width = 11, units = "in", res = 300)
print(CT_Fall_Survey_2025_population_percentages)
#dev.off()
...
#CT sites live
#CT_sites_live <- rbind(height_ASHC_Live,height_FENC_Live)
```
...
df_shellheight_proportions2026 <- height_ASHC_Live %>% 
  dplyr::filter(Year == "2026") %>% 
  filter(!is.na(Height)) %>% #Drop NAs early, ONLY for Height
  mutate(
    size_class = as.numeric(Height), 
    size_class_bin = cut(
      size_class, 
      breaks = seq(0, 210, by = 10), 
      # Fix 2: Create explicit range labels ("0-10", "10-20", etc.)
      labels = paste0(seq(0, 200, by = 10), "-", seq(10, 210, by = 10)), 
      include.lowest = TRUE, 
      right = TRUE
    )
  ) %>% 
  group_by(size_class_bin) %>% 
  dplyr::summarise(Count = n()) %>% 
  ungroup() %>% 
  mutate(Proportion = Count / sum(Count))

# Plotting the data
ggplot(df_shellheight_proportions2026, aes(x = size_class_bin, y = Proportion)) + 
  geom_col(fill = "steelblue", color = "black", width = 0.8) + 
  scale_y_continuous(labels = scales::percent_format()) + 
  # Fix 3: force ggplot to show bins even if Count is 0
  scale_x_discrete(drop = FALSE) + 
  labs( 
    title = "Ash Creek Shell Height Distribution (2026)", 
    x = "Size Class Bin (mm)", 
    y = "Proportion" 
  ) + 
  theme_minimal() + 
...
#CT Live 2024
```{r}
#CT live oyster shell height 2024 spring and fall to highlight growth and recruitment
CT_Distribution_Live_2024<- ggplot(data=CT_sites_live%>%filter(Year=="2024"), aes(x= Height))+
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
    labs(title="CT Oytsers Live Shell Height 2024", x ="Shell Height (mm)", y="Count")+ theme(axis.text.x = element_text(size=13,angle = 55,vjust =1.0, hjust=1.0))+theme(axis.text.y = element_text(size=13))+ theme(strip.text = element_text(size=15))+theme(plot.title = element_text(size=20))+theme(axis.title = element_text(size=15))+ scale_x_continuous(limits= c(0,190), breaks = seq(0,190, by =10) )+
  scale_y_continuous(limits = c(0,80), breaks = seq(0,80, by = 5))+
  facet_grid(
  factor(Season, levels = c("Spring", "Fall")) ~ Site, 
  scales = "free",axes = "all", axis.labels = "all_y"
) 
  #facet_wrap(~ factor(Season, levels = c("Spring", "Fall"))+ Site, scales = "free", nrow=2 , ncol=2)
pdf(paste0(path = "Field_Data_Biological/Output/CT_Distribution_Live_2024.pdf"), height = 7, width = 12)
print(CT_Distribution_Live_2024)
dev.off()
...
ASHC_Distribution_Dead<- ggplot(data=height_ASHC_Dead, aes(x= Height)) +
  ...
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Ash Creek Dead Oysters ", x ="Shell Height (mm)", y="Count")+ scale_x_continuous(limits= c(0,200), breaks = seq(0,200, by =10) )+
  scale_y_continuous(limits = c(0,20), breaks = seq(0,30, by = 5))+
   facet_wrap(~ factor(Season, levels = c("Spring", "Fall")) + Year, scales = "free", nrow=2 , ncol=4)
ASHC_Distribution_Dead
```
```{r}

ASHC_Distribution_Spring_Dead<-ggplot(data=height_ASHC_Dead %>% dplyr::filter(Season=="Spring"), aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  ...
  labs(title="Ash Creek Height Dead Oysters - Spring Survey ", x ="Height (mm)", y="Count")+ scale_x_continuous(limits= c(0,180), breaks = seq(0,180, by =10) )+
  scale_y_continuous(limits = c(0,25), breaks = seq(0,25, by = 5))+
   facet_wrap(~Year,  scales = "fixed", nrow=4, strip.position = "right") + # Moves labels to the left side
  theme(strip.placement = "outside")
ASHC_Distribution_Spring_Dead
#pdf(paste0(path = "Field_Data_Biological/Output/ASHC_Distribution_Spring_Dead.pdf"), height = 7, width = 12)
print(ASHC_Distribution_Spring_Dead)
#dev.off()
```
```{r}
ASHC_Distribution_Spring26_Dead <-ggplot(data=height_ASHC_Dead %>% dplyr::filter(Season=="Spring")%>% filter(Year =="2026"), aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(size=15))+theme(axis.text.y = element_text(size=15))+ theme(strip.text = element_text(size=15))+theme(plot.title = element_text(size=18))+theme(axis.title = element_text(size=15))+
  labs(title="Ash Creek Height Dead Oysters - Spring 2026 ", x ="Height (mm)", y="Count")+ scale_x_continuous(limits= c(0,180), breaks = seq(0,180, by =10) )+
  scale_y_continuous(limits = c(0,25), breaks = seq(0,25, by = 5))+
   facet_wrap(~Year,  scales = "fixed", nrow=4, strip.position = "right") + # Moves labels to the left side
  theme(strip.placement = "outside")
ASHC_Distribution_Spring26_Dead
```
#FENC
...
```{r}
FENC_Distribution_Spring26_Dead <-ggplot(data=height_FENC_Dead %>% dplyr::filter(Season=="Spring")%>% filter(Year =="2026"), aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(size=15))+theme(axis.text.y = element_text(size=15))+ theme(strip.text = element_text(size=15))+theme(plot.title = element_text(size=18))+theme(axis.title = element_text(size=15))+
  labs(title="Fence Creek Height Dead Oysters - Spring 2026 ", x ="Height (mm)", y="Count")+ scale_x_continuous(limits= c(0,180), breaks = seq(0,180, by =10) )+
  scale_y_continuous(limits = c(0,20), breaks = seq(0,20, by = 5))+
   facet_wrap(~Year,  scales = "fixed", nrow=4, strip.position = "right") + # Moves labels to the left side
  theme(strip.placement = "outside")
FENC_Distribution_Spring26_Dead

library(gridExtra)
spring26_dead <- grid.arrange(FENC_Distribution_Spring26_Dead,ASHC_Distribution_Spring26_Dead, nrow=2)

```

```{r}
CT_dead <- rbind(height_ASHC_Dead,height_FENC_Dead)

CT_spring26_dead_distrubtion <-ggplot(data=CT_dead %>% dplyr::filter(Season=="Spring")%>% filter(Year =="2026"), aes(x= Height)) +
  geom_histogram()+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(size=15))+theme(axis.text.y = element_text(size=15))+ theme(strip.text = element_text(size=15))+theme(plot.title = element_text(size=18))+theme(axis.title = element_text(size=15))+
  labs(title="Spring Survey 2026 - Dead Oysters ", x ="Shell Height (mm)", y="Count")+ scale_x_continuous(limits= c(0,180), breaks = seq(0,180, by =10) )+
  scale_y_continuous(limits = c(0,20), breaks = seq(0,20, by = 5))+
   facet_wrap(~Site,  scales = "fixed", nrow=4, strip.position = "right") + # Moves labels to the left side
  theme(strip.placement = "outside") 
CT_spring26_dead_distrubtion
#pdf(paste0(path = "Field_Data_Biological/Output/CT_spring26_dead_distrubtion.pdf"), height = 6, width = 12)
print(CT_spring26_dead_distrubtion)
#dev.off()
